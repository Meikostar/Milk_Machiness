 ...
 *
 */
package apache.mina.filter.keepalive;

import apache.mina.core.filterchain.IoFilter;
import apache.mina.core.filterchain.IoFilterAdapter;
import apache.mina.core.filterchain.IoFilterChain;
import apache.mina.core.service.IoHandler;
import apache.mina.core.session.AttributeKey;
import apache.mina.core.session.IdleStatus;
import apache.mina.core.session.IoEventType;
...
public class KeepAliveFilter extends IoFilterAdapter {
    ...
    public void messageReceived(NextFilter nextFilter, IoSession session, Object message) throws Exception {
        System.out.println("message received"+android.os.Process.myPid()+'-'+android.os.Process.myTid());
        try {
    ...
    public void messageSent(NextFilter nextFilter, IoSession session, WriteRequest writeRequest) throws Exception {
        System.out.println("message sent"+android.os.Process.myPid()+'-'+android.os.Process.myTid());
    ...
    public void sessionIdle(NextFilter nextFilter, IoSession session, IdleStatus status) throws Exception {
        if (status == interestedIdleStatus) {
            if (!session.containsAttribute(WAITING_FOR_RESPONSE)) {
                ...
                if (pingMessage != null) {
                    System.out.println("message write"+android.os.Process.myPid()+'-'+android.os.Process.myTid());
